...
FNeuron.getNeuronType = function(type)
{
	...
	else if (type == "K2TNeuron")
	{
		//return new KTNeuron(NeuronPar.x0, NeuronPar.y0, NeuronPar.K, NeuronPar.T, NeuronPar.z0);
		return new K2TNeuron(NeuronPar.getParameters());
	}
	else if (type == "K2TzNeuron")
	{
		//return new KTzNeuron(NeuronPar.x0, NeuronPar.y0, NeuronPar.z0, NeuronPar.K, NeuronPar.T, NeuronPar.d, NeuronPar.l, NeuronPar.xR);
		return new K2TzNeuron(NeuronPar.getParameters());
	}
...
K2TzNeuron.prototype.get_x = function()
{
	return this.x;
}

//function KTNeuron(x0, y0, K, T, H)
function K2TNeuron(neuPar)
{
	/*this.x = x0;
	this.y = y0;
	this.H = H;
...
function isTanhNeuron(neuType)
{
	return (neuType == "KTNeuron") || (neuType == "KTzNeuron") || (neuType == "K2TNeuron") || (neuType == "K2TzNeuron");
}

function has2Tanh(neuType)
{
	return neuType.indexOf("K2T") != -1;
}
...
function calcQuiescentIC()
{
	getParameters();
	var func;
	if (isTanhNeuron(NeuronPar.type))
	{
		func = Math.atanh;
	}
	else
	...
	if (has2Tanh(NeuronPar.type))
	{
		var yy0 = Math.tanh( (NeuronPar.x0 + NeuronPar.H)/NeuronPar.T );
		document.getElementById("y0").value = yy0;
		document.getElementById("z0").value = NeuronPar.T * func(NeuronPar.x0) + NeuronPar.K * yy0 - NeuronPar.x0; // x0 = y0
	}
	else
	{
